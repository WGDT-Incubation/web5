...
export default function IssuedPage() {
  ...
  const defaultMember: Record<string, string> =
    certificateParams.familyCertificate
      ...
      : {};
  const [membersData, setMembersData] = useState<Record<string, string>[]>([
    defaultMember,
  ]);
  ...
  useEffect(() => {
    ...
    if (matchedKey === "familyCertificate") {
      const { fields: familyFields, members: familyMembers } =
        certificateParams.familyCertificate as {
          fields: string[];
          members: string[];
        };

      // 1. populate the non-member fields
      const initialFormData = familyFields.reduce(
        (acc, field) => ({
          ...acc,
      ...
      // 2. try to pull a JSON-encoded “members” param
      let initialMembers: Record<string, string>[] = [];
      const jsonBlob = searchParams.get("members");
      if (jsonBlob) {
        try {
          initialMembers = JSON.parse(jsonBlob);
        } catch (_) {
          console.warn("Invalid members JSON");
        }
      }

      // 3. if that was empty, fall back to member_0_* parsing
      if (initialMembers.length === 0) {
        searchParams.forEach((value, key) => {
          ...
          const idx = parseInt(m[1], 10),
            field = m[2];
          initialMembers[idx] = initialMembers[idx] || {};
          initialMembers[idx][field] = value;
        });
      }

      if (initialMembers.length === 0) {
        initialMembers = [
          familyMembers.reduce((acc, f) => ({ ...acc, [f]: "" }), {}),
        ];
      }
      initialMembers = initialMembers.map((m) => ({
        ...m,
        disabled: m.disabled === "Y" || m.disabled === "true" ? "Y" : "N",
      }));

      setMembersData(initialMembers);
    } else {
      const fieldList = certificateParams[matchedKey] as string[];
    ...
    try {
      ...
      const filteredAllDIDs = allDIDs
        .map((item) => {
          const filteredDIDs = Object.entries(item.dids).filter(([_, did]) => {
            ...
            const certType = did.certificateType?.toLowerCase() || "";
            const issuerEmail = did.issuedBy || "";

            if (issuerEmail !== userEmail) return false;

            if (userEmail.includes("income.cert.issuer"))
              return certType.includes("income");
            if (userEmail.includes("family.cert.issuer"))
              return certType.includes("family");
            if (userEmail.includes("birth.cert.issuer"))
              return certType.includes("birth");
            if (userEmail.includes("migration.cert.issuer"))
              return certType.includes("migration");
      ...
      for (const item of filteredAllDIDs) {
        ...
        didKeys.forEach((didKey, idx) => {
          ...
          const fullName = `${
            didObj.name || didObj.studentName || didObj.applicantName || ""
          } `.trim();
  ...
  const handlePreview = async () => {
   ...
   if (
      selectedCertificate === "familyCertificate" &&
      membersData.length > 1 &&
      JSON.stringify(membersData[0]) === JSON.stringify(membersData[1])
    ) {
      setModalType("error");
      setModalMessage("Family Member 1 and 2 must not have identical data.");
    ...
    try {
      ...
      // setRows(matchedTemplate.dynamicKeys)
      // console.log('matchedTemplate.dynamicKeys)===>>>',matchedTemplate.dynamicKeys)
  ...
  const issueCertificateAction = async () => {
    ...
    try {
      ...
      if (!loginRes.ok) {
        console.error("Admin login failed, redirecting to login");
        router.push("/");
        return;
      }
      ...
      const enrichedEntry: Record<string, any> = {
        ...formData,           
        rows: tableRows,          
      ...
      if (!enrichedEntry.phoneNumber || !selectedCertificate) {
        throw new Error("Missing required fields: phone number or certificate type");
      }
      ...
      const edRes = await fetch(`${apiIp}admin/insertEDistrictDataBulk`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${authToken}`,
      ...
      if (!edRes.ok) {
        const errText = await edRes.text();
        throw new Error(`E-District insert failed: ${errText}`);
      }
  

      setModalType("success");
      setModalMessage("Certificate added successfully!");
      ...
      console.error(err);
      setModalType("error");
      setModalMessage(err.message || "Unexpected error—please try again.");
    } finally {
  ...
  const handleCSVImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      ...
      complete: (result: Papa.ParseResult<any>) => {
        ...
        if (selectedCertificate === "familyCertificate") {
  ...
  return (
    <div className="issued-container">
      <div className="bulk-container-top">
        <h1 className="issued-text">Add Certificate</h1>
        <button
          onClick={() => {
            setSelectedCertificate(null);
            setFormData({});
      ...
      <div className="template-dropdown-wrapper" ref={dropdownRef}>
        ...
        {showCertificateDropdown && (
          <div className="checkbox-group">
            {[
              ...new Set([
                ...Object.keys(filteredCertificateParams)
              ]),
            ].map((certName) => (
              <label key={certName}>
                <input
          ...
          Array.isArray(certificateParams[selectedCertificate]) ? (
            ...
            <>
              ...
              <div className="table-params-section">
                ...
                <button type="button" onClick={addTableRow} className="csv-upload-label" >
                  Add Row
                </button>
              </div>
            </>
...
// import { useSearchParams } from "next/navigation";
// import rawData from "../../app/data/pending.json";
// const data = rawData as Partial<DID>[];
// import Papa from "papaparse";
// import Image from "next/image";
// import successIcon from "../../../public/images/Success Mark.gif";
// import loadIcon from "../../../public/images/Document files.gif";
...
//   const [documents, setDocuments] = useState<DocumentData[]>([]);
//   const [currentPage, setCurrentPage] = useState<number>(1);
//   const [_jwt, setJwt] = useState<string>("");
//   const [tableRows, setTableRows] = useState<Record<string, string>[]>([]);
//   const [isLoading, setIsLoading] = useState<boolean>(true);
//   const router = useRouter();
//   const [previewHTML, setPreviewHTML] = useState<string>("");
...
//   const [storedTemplate, setStoredTemplate] =
//     useState<StoredTemplateData | null>(null);
//   const [allowedCertificateTypes, setAllowedCertificateTypes] = useState<
//     string[]
//   >([]);
...
//   //       const res = await fetch(`${apiIp}certificate/getAllCertificates`);
//   //       const result = await res.json();
//   //       console.log("Certificate Params Fetched:", result.data);
...
//   //       if (!certParamMap["familyCertificate"]) {
//   //         certParamMap["familyCertificate"] = {
//   //           fields: [
//   //             "familyId",
//   //             "date",
//   //             "address",
...
//   //       setCertificateParams(certParamMap);
//   //       console.log('certParamMap  ====>>>',certParamMap)
//   //     } catch (error) {
//   //       console.error("Error fetching certificate params:", error);
//   //     }
//   //   };
...
//     const spec = certificateParams[selectedCertificate];
//     if (typeof spec !== "object" || !("members" in spec)) return;
//     const blank = spec.members.reduce((acc, col) => ({ ...acc, [col]: "" }), {});
//     setTableRows(rs => [...rs, blank]);
//   };
...
//   const handleTableInputChange = (rowIdx: number, col: string, value: string) => {
//     setTableRows(rs =>
//       rs.map((r, i) => (i === rowIdx ? { ...r, [col]: value } : r))
//     );
//   };
...
//               "disabled",
//             ],
//             tableParams: [], // Ensure this is defined if missing
//           };
//         }
...
//         let certs: string[] = [];
//         if (Array.isArray(data.certificateTypes)) {
//           certs = data.certificateTypes.map((type: string) =>
//             type.toLowerCase()
//           );
//         }
...
//       .catch((err) => {
//         console.error("Failed to fetch allowed types:", err);
//         setAllowedCertificateTypes(["familycertificate"]);
//       });
//   }, []);
...
//   useEffect(() => {
//     if (!searchParams || !certificateParams) return;

//     const certType = searchParams.get("certificateType") || "";
//     const normalizedCertType = certType.toLowerCase().replace(/\s+/g, "");

//     const matchedKey = Object.keys(certificateParams).find(
//       (key) => key.toLowerCase().replace(/\s+/g, "") === normalizedCertType
//     );

//     if (!matchedKey) return;

//     const { fields: familyFields, members: familyMembers } =
//       certificateParams[matchedKey] as {
//         fields: string[];
//         members: string[];
//       };

//       const initialFormData = familyFields.reduce((acc, field) => {
//       acc[field] = searchParams.get(field) || "";
//       return acc;
//     }, {} as Record<string, string>);
//     setFormData(initialFormData);

//     const memberFieldPrefix = /^member_(\d+)_(.+)$/i;
//     const membersMap: Record<number, Record<string, string>> = {};
...
//     searchParams.forEach((value, key) => {
//       const match = key.match(memberFieldPrefix);
//       if (match) {
//         const index = parseInt(match[1], 10);
//         const rawField = match[2];
//         const normalized = rawField.toLowerCase().replace(/[^a-z]/g, "");
//         const finalField = aliasMap[normalized] || rawField;

//         if (!membersMap[index]) membersMap[index] = {};
//         membersMap[index][finalField] = value;
//       }
//     });

//     const membersArray = Object.keys(membersMap)
//       .sort((a, b) => Number(a) - Number(b))
//       .map((index) => membersMap[Number(index)]);

//     const initialMember = familyMembers.reduce(
//       (acc, m) => ({ ...acc, [m]: "" }),
//       {} as Record<string, string>
//     );

//     setMembersData(membersArray.length ? membersArray : [initialMember]);
//   }, [searchParams, certificateParams]);
...
//         item.phoneNumber === incomingPhone &&
//         item.certificateType?.toLowerCase().replace(/[\s-_]/g, "") ===
//           selectedCertificate.toLowerCase().replace(/[\s-_]/g, "")
//     );

//     if (entry) {
//       const raw = certificateParams[selectedCertificate!];
//       let fieldsList: string[];
//       if (Array.isArray(raw)) {
//         fieldsList = raw;
//       } else if (raw && typeof raw === "object" && Array.isArray(raw.fields)) {
//         fieldsList = raw.fields;
//       } else {
//         fieldsList = [];
//       }
//       const filledData = fieldsList.reduce((acc, field) => {
//         const safeEntry = entry as DID;
//         acc[field] = String(safeEntry[field as keyof DID] || "");
//         return acc;
//       }, {} as Record<string, string>);
...
//       });
//       if (!loginRes.ok) {
//         console.error("Admin login failed, redirecting to /");
//         router.push("/");
//         return;
//       }
//       const loginData = await loginRes.json();
...
//       setRawDIDs(didData.response.allDIDs);

//       const phoneMap: Record<string, DocumentData> = {};
//       let rowId = 1;

//       for (const item of didData.response.allDIDs) {
//         if (!item.dids || Object.keys(item.dids).length === 0) continue;
//         const didKeys = Object.keys(item.dids).sort((a, b) => {
//           const certA = item.dids[a].CertiNo || "";
//           const certB = item.dids[b].CertiNo || "";
...
//       doc.mobileNumber.toLowerCase().includes(searchQuery.toLowerCase()) ||
//       doc.didNumbers.some((did) =>
//         did.toLowerCase().includes(searchQuery.toLowerCase())
//       )
//   );
//   const PAGE_SIZE = 5;
//   const totalPages = Math.ceil(filteredDocs.length / PAGE_SIZE);
...
//     // ✅ FAMILY CERTIFICATE LOGIC
//     if (selectedCertificate === "familyCertificate") {
//       try {
//         const res = await fetch(`${apiIp}certificate/customFamilyCertificate`, {
//           method: "POST",
//           headers: {
//             "Content-Type": "application/json",
//           },
...
//         setShowPreview(true);
//       } catch (err) {
//         console.error("Error fetching family certificate preview:", err);
//         setCertificateHTML("<p>Error generating preview.</p>");
//         setShowPreview(true);
//       }
//       return;
//     }

//     // ✅ Default certificate preview logic (unchanged)
//     const endpoint = `${apiIp}certificate/getCustomCertificate`;
...
//       // Step 1: Admin login to get fresh token
//       const loginRes = await fetch(`${apiIp}admin/adminLogin`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
...
//       if (!enrichedEntry.phoneNumber || !selectedCertificate) {
//         throw new Error(
//           "Missing required fields: phone number or certificate type"
//         );
//       }

//       if (selectedCertificate === "familyCertificate" && membersData.length) {
//         enrichedEntry.members = membersData;
//       }
...
//       if (!payload.data.length || Object.keys(enrichedEntry).length === 0) {
//         throw new Error("No data to insert. Please fill out the form.");
//       }

//       // Step 5: Insert to eDistrict
//       const edRes = await fetch(`${apiIp}admin/insertEDistrictDataBulk`, {
//         method: "POST",
//         headers: {
//           "Content-Type": "application/json",
...
//           const memberMatch = key.match(/^member_(\d+)_(.+)$/i);
//           if (memberMatch) {
//             const memberIndex = parseInt(memberMatch[1], 10);
//             let memberFieldRaw = memberMatch[2];

//             // Normalize field name
//             const normalizedField = memberFieldRaw.toLowerCase().replace(/[^a-z]/g, "");
...
//             const finalField = aliasMap[normalizedField] || memberFieldRaw;

//             if (!updatedMembersData[memberIndex]) updatedMembersData[memberIndex] = {};

//             let cleanedVal = val;

//             if (finalField === "disabled") {
//               cleanedVal = val.toLowerCase().startsWith("y") ? "Y" : "N";
//             }

//             if (finalField === "mobile") {
//               cleanedVal = val.replace(/\D/g, "");
//             }

//             updatedMembersData[memberIndex][finalField] = cleanedVal;
//             console.log(`✅ Assigned to member[${memberIndex}]: ${finalField} = ${cleanedVal}`);
//             continue;
//           }
...
//           updatedFormData[finalKey] = val;
//           console.log(`✅ Assigned to form: ${finalKey} = ${val}`);
//         }
...
//     let fieldsList = Array.isArray(raw) ? raw : raw.fields;
//     let csvHeaders = ["certificateType", "mobileNumber"];

//     if (
//       selectedCertificate.toLowerCase().replace(/[^a-z]/g, "") ===
//       "familycertificate"
//     ) {
//       const familyFields = (raw as { fields: string[] }).fields || [];
//       const memberFields = (raw as { members: string[] }).members || [];

//       const memberHeaders = memberFields.map((field) => `member_0_${field}`);
//       csvHeaders = [...csvHeaders, ...familyFields, ...memberHeaders];
//     } else {
//       const paramsList = fieldsList.filter((f: string) => f !== "did");
//       csvHeaders = [...csvHeaders, ...paramsList];
//     }
...
//         {showCertificateDropdown && (
//           <div className="checkbox-group">
//             {Object.keys(filteredCertificateParams).map((certName:any) => (
//               <label key={certName}>
//                 <input
//                   type="checkbox"
//                   value={certName}
...
//                       if (certName === "familyCertificate" && selectedParam.members) {
//                         const blankMember = selectedParam.members.reduce(
//                           (mAcc: Record<string, string>, mField) => ({
//                             ...mAcc,
//                             [mField]: "",
...
//              <>
//                {/* scalar fields */}
//                {((certificateParams.familyCertificate as any).fields as string[])
//                  .filter((f) => f !== "did")
//                  .map((field) => {
...
//                            name={field}
//                            className="search-input-credential"
//                            placeholder={`Enter ${formatCertificateLabel(field)}`}
//                            value={(() => {
...
//                    {(
//                      (certificateParams.familyCertificate as any)
//                        .members as string[]
//                    ).map((mf) => {
//                      const val = member[mf] || "";
//                      const isDate = mf.toLowerCase().includes("date");
//                      const isBool = mf.toLowerCase().includes("disable");
//                      const isMob = mf.toLowerCase().includes("mobile");
//                      return (
//                        <div key={mf} className="issued-credential-container">
//                          <label>{formatCertificateLabel(mf)}</label>
//                          {isDate ? (
//                            <input
//                              type="date"
//                              name={`member-${idx}-${mf}`}
...
//                              name={`member-${idx}-${mf}`}
//                              className="search-input-credential"
//                              value={(member as any).disabled || ""}
//                              onChange={(e) =>
//                                setMembersData((ms) =>
//                                  ms.map((m, i) =>
//                                    i === idx
//                                      ? { ...m, disabled: e.target.value }
//                                      : m
//                                  )
//                                )
//                              }
...
//                    <button
//                      type="button"
//                      className="remove-btn-member"
//                      onClick={() =>
//                        setMembersData((ms) => ms.filter((_, i) => i !== idx))
//                      }
//                    >
//                      Remove
//                    </button>
...
//                  onClick={() => {
//                    const fields = (certificateParams.familyCertificate as any)
//                      .members as string[];
//                    setMembersData((ms) => [
//                      ...ms,
...
//                      <div key={idx} className="table-row1">
//                        {(
//                          certificateParams[selectedCertificate] as TabularSpec
//                        ).members.map((col) => (
//                          <div key={col} className="issued-credential-container">
...
//                              value={row[col] || ""}
//                              onChange={(e) =>
//                                handleTableInputChange(idx, col, e.target.value)
//                              }

//                            />
//                          </div>
//                        ))}
//                        <button type="button" onClick={() => removeTableRow(idx)}>
//                          Remove Row
//                        </button>
//                      </div>
//                    ))}
...
//                 setModalType("confirm");
//                 setModalMessage(
//                   "Do you want to add this certificate to issue?"
//                 );
//                 setModalVisible(true);
//               }}
//             >
